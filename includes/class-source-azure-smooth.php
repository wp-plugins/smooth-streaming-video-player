<?php
/**
 * Microsoft Azure Smooth Streaming videos source management.
 * 
 * @author Adenova <agence@adenova.fr>
 * @since 1.5.0
 */
if ( ! class_exists ( 'SVP_Source_Azure_Smooth' ) )
{
	require_once( 'class-source-base.php' );
	
	// It's a source for adaptive videos : include the adaptive video type class
	require_once( 'class-video-adaptive.php' );
	
	class SVP_Source_Azure_Smooth extends SVP_Source_Base
	{
		// Constructor
		function SVP_Source_Azure_Smooth()
		{
			$this->__construct();
		}
		...
		{
			$svp_video_adaptive = new SVP_Video_Adaptive();
			$this->set_videos_type( $svp_video_adaptive->get_type() );
		}
							...
							<tr valign="top">
								...
								<td>
									<label for="svp_source_compatibility_iphone">
										<input name="svp_source_compatibility_iphone" type="checkbox" id="svp_source_compatibility_iphone" value="1"' . $iphone_compatibility_checked . ' />' . __( 'Activate', 'svp-translate' ) . '
									</label>
								</td>
							</tr>
							<tr valign="top">
								<th scope="row">
									<label for="svp_source_iphone_dirname">' . __( 'Videos container name for iPhone', 'svp-translate' ) . '</label>
								</th>
								<td>
									...
									<br />' . __( 'This is the container name that contains your videos for iPhone or iPad on the server. This field must be filled if the iPhone compatibility option has been enabled.', 'svp-translate' ) . '
								</td>
							</tr>
						</table>
					</div>
							...
							<tr valign="top">
								...
								<td>
									<label for="svp_source_cdn_activation">
										<input name="svp_source_cdn_activation" type="checkbox" id="svp_source_cdn_activation" value="1"' . $cdn_activation_checked . ' />' . 
		...
		function scan( $id = null )
		{
			...
			$result = $videos->filter_by_extensions( $result, $svp_video_adaptive->get_extensions() );
		...
		function make_video_url( $filename = '' )
		{
			...
			switch ( $user_agent )
			{
				...
				case SVP_USER_AGENT_IPAD:
					...
					{
						if ( array_key_exists( 'svp_source_iphone_dirname', $options ) && ! empty( $options['svp_source_iphone_dirname'] ) )
						{
							$url .= $svp_utils->add_endurl_slash( $options['svp_source_iphone_dirname'] );
							$pos = strrpos( $filename, '.' );
							if ( $pos !== false )
							{
								$url .= urlencode( substr( $filename, 0, $pos ) . '-' . 
									SVP_VIDEO_SUFFIX_TS . '.' . 
									SVP_VIDEO_EXT_TS );
							}
							else
