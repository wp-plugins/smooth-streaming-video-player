 ...
 * @since 1.5.0
 */
if ( ! class_exists ( 'SVP_Source_IIS_Smooth' ) )
{
	...
	class SVP_Source_IIS_Smooth extends SVP_Source_Base
	{
		
		function SVP_Source_IIS_Smooth()
		{
			$this->__construct();
		}
		...
		function scan( $id = null )
		{
			...
			if ( $directory != '.' )
				$path = $directory . '/';
			$result = $videos->filter_by_extensions( $result, $svp_video_adaptive->get_extensions(), $path );
						...
						if ( array_key_exists( 'svp_source_iphone_dirname', $options ) && ! empty( $options['svp_source_iphone_dirname'] ) )
						{
							...
							if ( $pos !== false )
							{
								$url .= urlencode( substr( $filename, 0, $pos ) . '-' . 
									SVP_VIDEO_SUFFIX_TS . '.' . 
									SVP_VIDEO_EXT_SMOOTH ) . 
									'/Manifest(format=' . SVP_VIDEO_SUFFIX_TS . ')';
							}
							else
								return '';
						}
